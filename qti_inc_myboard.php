...
if ( $intMyTopics>0 ) {
  echo '<a class="button" href="'.url('qti_items.php').'?fq=user&fw='.SUser::id().'&fv='.urlencode(SUser::name()).'">'.L('All_my_items').'&nbsp;('.$intMyTopics.')</a>';
} else {
  echo '<span>'.L('None').'</span>';
}
if ( SUser::isStaff() && $intMyAssign>0 ) {
  echo '<a class="button" href="'.url('qti_items.php').'?fq=actor&fw='.SUser::id().'&fv='.urlencode(SUser::name()).'">'.L('Handled_by').' '.L('me').'&nbsp;('.$intMyAssign.')</a>';
}
echo '</div>'.PHP_EOL;
...
if ( $intMyTopics>0 )
{
  ...
  if ( is_array($row) )
  {
    ...
    echo '<p class="date">'.qtDate($oT->firstpostdate,'$','$',true,true).', '.($oT->firstpostuser==SUser::id() ? L('I_wrote') : L('By').' '.qtTrunc($oT->firstpostname,20)).'</p>'.PHP_EOL;
    ...
    if ( $oT->items>1 ) echo '<p class="date">'.L('reply',$oT->items).', '.L('last_message').': '.qtDate($oT->lastpostdate,'$','$',true,true).', '.($oT->lastpostuser==SUser::id() ? L('I_wrote') : L('By').' '.qtTrunc($oT->lastpostname,20)).'</p>'.PHP_EOL;
