<?php

echo '<p class="bold">Application gérée par</p>
<p>',$_SESSION[QT]['site_name'],'</p>
<p>Webmaster: <a href="mailto:',$_SESSION[QT]['admin_email'],'">',$_SESSION[QT]['admin_email'],'</a></p>
<p>Contact: ',$_SESSION[QT]['admin_name'],' ',$_SESSION[QT]['admin_addr'],'</p>
<br>
<p class="bold">Application créée par</p>
<p>QT-cute (www.qt-cute.org) version ',VERSION,'</p>
<br>
<p class="bold">Licences (anglais)</p>
<p><img src="bin/css/vgplv3.png" width="88" height="31" alt="GPL" title="GNU General Public License"/></p>
<p>Voyez les documents <a href="license.txt">Application License</a> et <a href="license_gpl.txt">GNU General Public License</a> pour de plus amples informations.</p>
<br>
<p class="bold">Conformités</p>
';