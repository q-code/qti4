...
private $type; // framework used to connect database engine (must be in lowercase). Ex: pdo.mysql
...
public $stats; // [NULL|array] timer and query counter (NULL means no stats). startStats() initializes the array.
...
public function startStats()
{
  ...
  $this->stats = ['num'=>0, 'start'=>$t, 'pagestart'=>$t, 'rows'=>0];
}
...
{
  return $this->type==='pdo.sqlite' || $this->type==='sqlite' ? '(local file) '.$this->db : $this->host;
}
/**
