...
if ( $row=$oDBAJAX->getRow() )
{
  ...
  echo '<p class="preview_date ellipsis">'.L('Created').': '.qtDate($row['firstpostdate'],'M d','',true).'</p>';
  if ( !empty($row['wisheddate']) ) echo '<p class="preview_date ellipsis">'.L('Wisheddate').': '.qtDate($row['wisheddate'],'M d','',true).'</p>';
