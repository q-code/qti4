...
public function renderInspectionResult(CSection $oS, CTopic $oT, bool $avatar=true, string $strEndLine='', string $strSkin='skin/default')
{
  ...
  $msg .= $str.'</p>'.PHP_EOL;
  // attachements
  if ( !empty($this->attach) ) $msg .= '<p class="small">'.qtSVG('paperclip', 'title='.L('Attachment')).' <a href="'.QT_DIR_DOC.$this->attach.'" class="attachment" target="_blank">'.$this->getSrcAttach().'</a></p>';
  // signature
  if ( $_SESSION[QT]['viewmode']!='C' && $this->type!='F' && !empty($this->usersign) )
  {
    $msg .= '<p class="post-sign">'.qtBBcode($this->usersign).'</p>'.PHP_EOL;
  ...
  <div class="inspection-date">'.$strIcon.' '.qtDate($this->issuedate,'$','$',true,true,true).'</div>
