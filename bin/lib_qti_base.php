...
function renderEmail($emails, string $mode='txt', int $size=0, string $failed='')
{
  ...
  switch($mode) {
    ...
    case 'symbol': return '<a href="mailto:'.$mailto.'" title="'.$emails[0].(isset($emails[1]) ? ', ...' : '').'">'.qtSvg('#envelope').'</a>';
  }
  ...
  switch($mode) {
    ...
    case 'symboljava': return '<a href="javascript:void(0)" onmouseover="qtEmailShow(this);" onmouseout="qtEmailHide(this);" data-emails="'.$mailto.'">'.qtSvg('#envelope').'</a>';
  }
...
function makePager(string $uri, int $count, int $pnsize=50, int $currentpage=1, string $sep='', string $currentclass='current')
{
  ...
  if ( $pnsize<5 ) $pnsize=50;
  if ( $count<2 || $count<=$pnsize ) return ''; //█
