...
switch($q) {
  ...
  case 'qkw':
    ...
    $oDB->query(
        "SELECT t.id,t.numid,t.type,p.title,p.textmsg,s.numfield,p.type as posttype FROM TABTOPIC t INNER JOIN TABPOST p ON p.topic=t.id INNER JOIN TABSECTION s ON s.id=t.section WHERE $where",
        [':v'=>$bRef ? (int)$fv : '%'.$fv.'%']
        );
    while($row=$oDB->getRow())
    {
      ...
      if ( $row['type']==='A' ) $image='thumbtack fa-rotate-270';
      if ( $bRef )
      {
        if ( empty($row['title']) ) $row['title']=substr($row['textmsg'],0,30);
        if ( !isset($arr[$id]) )
          $arr[$id] = array(
              'rItem'=>'<i class="ajax fa fa-'.$image.'"></i> '.$ref,
    ...
    while($row=$oDB->getRow())
    {
      ...
      foreach($arrTags as $str)
      {
        if ( stripos($str, $fv)!==false )
        {
          ...
          {
            $arrDistinct[$str] = $row['countid'];
          }
        }
        if ( count($arrDistinct)>8 ) break;
      }
    }
    foreach($arrDistinct as $key=>$i) $arr[]=array('rItem'=>$key,'rInfo'=>$i);
    // search in predefined tags
    if ( !empty($_GET['src']) && count($arr)<10 && $s>=0 ) {
      require 'lib_qt_tags.php';
      // search matching in section tags
      $arrTags = readTagsFile($_GET['src'].'_'.$s.'.csv');
      ...
      if ( count($arrDistinct)<10 ) {
        $arrTags = readTagsFile($_GET['src'].'.csv');
        foreach($arrTags as $str=>$strDesc) {
          if ( stripos($str, $_GET['fv'])!==false ) $arrDistinct[$str] = substr($strDesc,0,64);
          if ( count($arrDistinct)>10 ) break;
        }
  ...
  case 'kw':
    ...
    while($row=$oDB->getRow()) {
      $id = (int)$row['id'];
      //$format = empty($row['numfield']) || $row['numfield']=='N' ? '%03s' : $row['numfield'];
      ...
      if ( !isset($arr[$id]) )
        $arr[$id] = array(
            'rItem'=>'<i class="ajax fa fa-'.$image.'"></i>'.$str,
            'rInfo'=>$row['title'],
            'rSelect'=>$row['textmsg']
        );
        if ( count($arr)>8 ) break;
