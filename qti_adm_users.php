...
// Lettres bar
if ( $intCount>$ipp || $fg!=='all' ) echo htmlLettres(url($oH->php).qtURI('group|page'), $fg, L('All'), 'lettres', L('Username').' '.L('starting_with').' ', $intCount>300 ? 1 : ($intCount>$ipp*2 ? 2 : 3));
...
// Build paging
$paging = makePager("qti_adm_users.php?cat=$strCateg&fg=$fg&po=$strOrder&pd=$strDirec",$intCount,$ipp,$pn);
...
$t->arrTh['role'] = new TabHead(L('Role'), 'class=c-role', '<a href="'.$oH->php.'?cat='.$strCateg.'&fg='.$fg.'&po=role&pd=asc">%s</a>');
