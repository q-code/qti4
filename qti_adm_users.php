...
<td>'.($cat==='all' ? $icon[2] : '<a href="qti_adm_users.php" title="'.L('Show').'">'.$icon[1].'</a>').'</td>
</tr>
';
echo '<tr>
...
<td>'.($intFalse==0 ? $icon[0] : ($cat==='FM' ? $icon[2] : '<a href="qti_adm_users.php?cat=FM" title="'.L('Show').'">'.$icon[1].'</a>')).'</td>
</tr>
';
echo '<tr>
...
<td>'.($intSleeping==0 ? $icon[0] : ($cat==='SM' ? $icon[2] : '<a href="qti_adm_users.php?cat=SM" title="'.L('Show').'">'.$icon[1].'</a>')).'</td>
</tr>
';
echo '</table>
';
...
echo '<p style="margin:12px 0">'.($cat=='all' ? '' : '<a href="qti_adm_users.php">'.qtSvg('chevron-left').L('Show').' '.L('all').'</a> | ');
...
if ( $cat!='all' ) {
  ...
  echo '<br><h1 class="title">'.L('Users_'.$cat).' <span style="font-size:11pt">('.L('h_Users_'.$cat).': '.$intCount.')<span></h1>'.PHP_EOL;
}
...
echo htmlLettres(url($oH->php).qtURI('group|page'), $fg, L('All'), L('Username').' '.L('starting_with').' ', $intCount>300 ? 1 : ($intCount>$ipp*2 ? 2 : 3));
...
// Build paging
$paging = makePager("qti_adm_users.php?cat=$cat&fg=$fg&po=$po&pd=$pd",$intCount,$ipp,$pn);
if ( !empty($paging) ) $paging = L('Page').$paging;
if ( $intCount<$intUsers ) $paging = '<small>'.L('user',$intCount).' ('.L('from').' '.$intUsers.')</span>'.(empty($paging) ? '' : ' | '.$paging);
...
$t->arrTh['role'] = new TabHead(L('Role'), 'class=c-role', '<a href="'.$oH->php.'?cat='.$cat.'&fg='.$fg.'&po=role&pd=asc">%s</a>');
...
//-- LIMIT QUERY --
$strState = 'id,name,closed,role,numpost,firstdate,lastdate,ip FROM TABUSER WHERE id>'.($cat=='all' ? '0' : '1').$sqlWhere;
...
while($row=$oDB->getRow()) {
  ...
  $t->arrTd['name']->content = '<a href="'.url('qti_user.php').'?id='.$row['id'].'" title="'.qtAttr($row['name'],24).'">'.qtTrunc($row['name'],24).'</a>';
...
// Extra command
if ( $cat!=='all' ) {
  echo '<p class="submit"><a href="'.APP.'_adm_register.php?a=catdel&cat='.$cat.'&n='.$intCount.'">'.L('Delete').' '.L('users_'.$cat).'...</a></p>';
}
