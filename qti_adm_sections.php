...
foreach($arrDomains as $idDomain=>$domain) {
  ...
  echo '<td class="group handler">'.(count($arrDomains)<2 ? '' : '<span class="draghandler" title="'.L('Move').'" onclick="qtToggle(`dlg-reorder`)">'.getSVG('arrows-v').'</span>').'</td>'.PHP_EOL;
  echo '<td class="group c-section" colspan="2">'.$domain.'</td>'.PHP_EOL;
  echo '<td class="group">&nbsp;</td>'.PHP_EOL;
  echo '<td class="group">&nbsp;</td>'.PHP_EOL;
  echo '<td class="group c-action"><a href="'.APP.'_adm_domain.php?id='.$idDomain.'" title="'.L('Edit').'">'.getSVG('pen-square').'</a>';
  echo ' &middot; '.($idDomain===0 ? '<span class="disabled" title="'.L('Delete').'">'.getSVG('trash').'</span>' : '<a href="'.APP.'_dlg_adm.php?a=Ddelete&s='.$idDomain.'" title="'.L('Delete').'">'.getSVG('trash').'</a>');
  echo ' &middot; ';
  $strUp = getSVG('caret-up', 'class=disabled');
  $strDw = getSVG('caret-down', 'class=disabled');
  if ( count($arrDomains)>1 ) {
    if ( $i>0 ) $strUp = '<a class="popup_ctrl" href="'.$oH->selfurl.'?d='.$idDomain.'&a=d_up" title="'.L('Up').'">'.getSVG('caret-up').'</a>';
    if ( $i<count($arrDomains)-1 ) $strDw = '<a class="popup_ctrl" href="'.$oH->selfurl.'?d='.$idDomain.'&a=d_down" title="'.L('Down').'">'.getSVG('caret-down').'</a>';
  }
  ...
  if ( isset($arrSections[$idDomain]) && count($arrSections[$idDomain])>0 ) {
    ...
    foreach($arrSections[$idDomain] as $idSection=>$arrSection) {
      ...
      echo '<td class="handler">'.($isSortable ? '<span class="draghandler" title="'.L('Move').'" draggable="true">'.getSVG('arrows-v').'</span>' : '').'</td>'.PHP_EOL;
      ...
      echo '<td class="c-action"><a href="'.APP.'_adm_section.php?s='.$oS->id.'" title="'.L('Edit').'">'.getSVG('pen-square').'</a>';
      echo ' &middot; '.($idSection===0 ? '<span class="disabled" title="'.L('Delete').'">'.getSVG('trash').'</span>' : '<a href="'.APP.'_dlg_adm.php?a=Sdelete&s='.$idSection.'" title="'.L('Delete').'">'.getSVG('trash').'</a>');
      echo ' &middot; ';
      if ( count($arrSections[$idDomain])>1 ) {
        if ( $j>0 ) $strUp = '<a href="'.$oH->selfurl.'?d='.$idDomain.'&s='.$idSection.'&a=s_up" title="'.L('Up').'">'.getSVG('caret-up').'</a>';
        if ( $j<count($arrSections[$idDomain])-1 ) $strDw = '<a href="'.$oH->selfurl.'?d='.$idDomain.'&s='.$idSection.'&a=s_down" title="'.L('Down').'">'.getSVG('caret-down').'</a>';
      }
      echo $strUp.'&nbsp;&thinsp;'.$strDw;
      $j++;
      echo '</td></tr>'.PHP_EOL;
