...
if ( QT_SHOW_MODERATOR && isset($oS) && is_a($oS,'CSection') && $oS->id>=0 ) {
  if ( !empty($oS->ownerid) && !empty($oS->ownername) ) $arr[1] = L('Role_C').': <a href="'.Href('qti_user.php?id='.$oS->ownerid).'">'.$oS->ownername.'</a>';
}
echo '<div id="main-ft">
...
if ( QT_SHOW_JUMPTO ) {
  ...
  if ( $oH->selfurl==='qti_search.php' ) echo '<option value="'.Href('qti_index.php').'">'.SLang::translate().'</option>';
  if ( $oH->selfurl!=='qti_search.php' && SUser::canView('V4') ) echo '<option value="'.Href('qti_search.php').'">'.L('Advanced_search').'</option>';
  echo sectionsAsOption('',[],[],'',32,100,Href('qti_items.php').'?s='); // current section is not rejected (allow returning to page 1 or top page)
  echo '</select>';
}
echo '</p>
</div>
...
$oH->scripts[] = 'function iniAsideState(){
  ...
  try {
    const isExpanded = localStorage.getItem("qti_aside")==="on";
...
function storeAsideState(){
  ...
  try {
    localStorage.setItem("qti_aside", d.classList.contains("expanded") ? "on" : "off");
  } catch {
    console.log("localStorage not available"); return;
  }
}
...
if ( SUser::role()!=='V' || SUser::role().substr($_SESSION[QT]['m_rss_conf'],0,1)=='VV' ) {
  $arrMenus['rss'] = 'text='.getSVG('rss-square').'|id=menu-rss|href=qtim_rss.php';
}}

// footer menu extra definition
if ( SUser::role()==='A' ) {
  $arrMenus['admin'] = '['.L('Administration').']|id=menu-admin|href=qti_adm_index.php|accesskey=a';
}
