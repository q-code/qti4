...
foreach (LANGUAGES as $iso=>$lang)
{
  ...
  $m['pan-'.$iso] = $lang.'|href='.$oH->php.'?pan='.$iso.'|id=pan-'.$iso.'|class=pan-tab|title='.L('Edit').' '.$lang;
}
$m = new CMenu($m,'');
...
echo '<td class="c-file">'.($bFile ? $file.' <a href="'.$oH->php.'?pan='.$pan.'&a=view&file='.QT_DIR_DOC.$file.'" title="'.L('Preview').'">'.qtSVG('search').'</a> <a href="'.QT_DIR_DOC.$file.'" title="'.L('download').'">'.qtSVG('download').'</a>' : '<a href="tool_txt.php?exit='.urlencode($oH->php.'?pan='.$pan).'&file='.QT_DIR_DOC.$file.'" title="'.L('Add').'...">'.qtSVG('magic').'</a>').'</td>';
echo '<td class="c-action">';

echo $bFile ? '<a href="tool_txt.php?exit='.urlencode($oH->php.'?pan='.$pan).'&file='.QT_DIR_DOC.$file.'">'.L('Edit').'</a>' : '<span class="disabled">'.L('Edit').'</span>';
echo '<br>';
echo '<a href="'.APP.'_adm_tags_upload.php?pan='.$pan.'&v='.$file.'">'.L('Upload').'</a>';
echo '<br>';
echo $bFile ? '<a href="tool_txt.php?exit='.urlencode($oH->php.'?pan='.$pan).'&a=delete&file='.QT_DIR_DOC.$file.'">'.L('Delete').'</a>' : '<span class="disabled">'.L('Delete').'</span>';
...
foreach($arrDomains as $idDom=>$strDomtitle)
{
  ...
  foreach($arrSections[$idDom] as $intSecid=>$arrSection)
  {
    ...
    echo '</span></td>';
    echo '<td class="c-file">'.($bFile ? $file.' <a href="'.$oH->php.'?pan='.$pan.'&s='.$oS->id.'&a=view&file='.QT_DIR_DOC.$file.'" title="'.L('Preview').'">'.qtSVG('search').'</a> <a href="'.QT_DIR_DOC.$file.'" title="'.L('download').'">'.qtSVG('download').'</a>' : '<a href="tool_txt.php?exit='.urlencode($oH->php.'?pan='.$pan).'&file='.QT_DIR_DOC.$file.'" title="'.L('Add').'...">'.qtSVG('magic').'</a>').'</td>';
    echo '<td class="c-action">';
