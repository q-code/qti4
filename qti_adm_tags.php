...
echo '<td class="c-file">'.($bFile ? $file.' <a href="'.$oH->selfurl.'?pan='.$pan.'&a=view&file='.QT_DIR_DOC.$file.'" title="'.L('Preview').'">'.getSVG('search').'</a> <a href="'.QT_DIR_DOC.$file.'" title="'.L('download').'">'.getSVG('download').'</a>' : '<a href="tool_txt.php?exit='.urlencode($oH->selfurl.'?pan='.$pan).'&file='.QT_DIR_DOC.$file.'" title="'.L('Add').'...">'.getSVG('magic').'</a>').'</td>';
...
foreach($arrDomains as $idDom=>$strDomtitle)
{
  ...
  foreach($arrSections[$idDom] as $intSecid=>$arrSection)
  {
    ...
    echo '</span></td>';
    echo '<td class="c-file">'.($bFile ? $file.' <a href="'.$oH->selfurl.'?pan='.$pan.'&s='.$oS->id.'&a=view&file='.QT_DIR_DOC.$file.'" title="'.L('Preview').'">'.getSVG('search').'</a> <a href="'.QT_DIR_DOC.$file.'" title="'.L('download').'">'.getSVG('download').'</a>' : '<a href="tool_txt.php?exit='.urlencode($oH->selfurl.'?pan='.$pan).'&file='.QT_DIR_DOC.$file.'" title="'.L('Add').'...">'.getSVG('magic').'</a>').'</td>';
    echo '<td class="c-action">';
...
echo '<h2 style="margin:20px 0 10px 0">'.getSVG('search').' '.L('Preview').'</h2>';
