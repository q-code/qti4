...
$strParticip = '';
if ( $items>0 ) {
$strParticip .= '<a href="'.url('qti_items.php').'?fq=user&fw='.$id.'&fv='.urlencode($row['name']).'">'.L('Item',$items).'</a>, ';
}
if ( $countmessages>0 ) {
  $strParticip .= '<a href="'.url('qti_items.php').'?fq=userm&fw='.$id.'&fv='.urlencode($row['name']).'">'.L('Message',$countmessages).'</a>';
