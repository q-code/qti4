...
echo '<p style="margin:0 0 10px 0;text-align:right"><a id="tgl-ctrl" class="tgl-ctrl" href="javascript:void(0)" onclick="qtToggle();this.classList.toggle(`expanded`);">'.L('Unreplied').qtSVG('angle-down','','',true).qtSVG('angle-up','','',true).'</a></p>
...
foreach($arrDomains as $idDom=>$strDomtitle)
{
  ...
  foreach(CSection::getIdsInContainer($idDom) as $s) {
    ...
    echo '<td class="c-section"><span class="sectionname">'.$oS->title.'</span><br><small>'.L('Section_type.'.$oS->type).($oS->status==='1' ? ', '.L('Section_status.1') : '').'</span></td>';
