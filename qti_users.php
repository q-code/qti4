...
if ( SUser::canAccess('show_calendar') )echo ' &middot; <a href="'.Href('qti_calendar.php').'" style="white-space:nowrap">'.L('Birthdays_calendar').'</a>';
...
for ($i=0;$i<($_SESSION[QT]['viewmode']=='C' ? 2 : 5);++$i)
{
  $row = $oDB->getRow();
  if ( !$row ) break;
  echo '<tr><td><a href="'.Href('qti_user.php').'?id='.$row['id'].'">'.$row['name'].'</a></td><td class="right">'.qtIntK((int)$row['numpost']).'</td></tr>';
}
echo '</table>
</div>';
...
// -- build paging --
$strPaging = makePager( Href('qti_users.php?group='.$pageGroup.'&order='.$pageOrder.'&dir='.$pageDirec), $intCount, (int)$_SESSION[QT]['items_per_page'], $intPage );
...
if ( !$bCompact ) $t->arrTh['userphoto']  = new TabHead(getSVG('camera'));
...
if ( SUser::isStaff() ) $t->arrTh['userpriv'] = new TabHead(getSVG('info'), 'title='.L('Privacy'));
...
while($row=$oDB->getRow())
{
  ...
  $t->arrTd['username']->content = '<a href="'.Href('qti_user.php').'?id='.$row['id'].'">'.QTtrunc($row['name'],24).'</a>';
  ...
  $t->arrTd['userpriv']->content = renderUserPrivSymbol($row);
	...
	if ( $bMap && !QTgempty($row['x']) && !QTgempty($row['y']) )
	{
		...
		$strPname = $row['name'];
		$strPinfo = $row['name'].'<br><a class="gmap" href="'.Href('qti_user.php').'?id='.$row['id'].'">Open profile &raquo;</a>';
