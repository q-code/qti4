<?php  //4.0 build:20230205 allows app impersonation [qt f|i ]
...
if ( isset($_POST['ok']) ) try {
  ...
  foreach($_POST as $k=>$posted) {
    if ( substr($k,0,3)=='tr-' && !empty($posted) ) {
      ...
      SLang::add('index', substr($k,3), 'i', $posted);  // will encode
    }
  }
  memFlushLang(); // Clear cache
  SMem::set('settingsage',time());
  $_L['index'] = SLang::get('index',QT_LANG); // Register lang
  $_SESSION[QT.'splash'] = L('S_save'); // Exit
...
if ( $strHelper ) $warning .= $strHelper;// '<p>'.getSVG('flag', 'style=font-size:1.4rem;color:#1364B7').' '.$strHelper.'</p>';
if ( !empty($warning) ) $warning = getSVG('flag', 'style=font-size:1.4rem;color:#1364B7').' '.$warning;
...
<tr>
<td colspan="2" style="background-color:transparent">* <small>'.L('E_no_translation').'<strong style="color:#1364B7">'.$_SESSION[QT]['index_name'].'</strong></small></td>
</tr>
</table>
';
