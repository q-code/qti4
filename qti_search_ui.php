...
if ( SUser::id()>0 ) echo '<a id="btn_my" data-s="'.$s.'" data-st="'.$st.'" class="button" href="'.url(APP.'_items.php').'?q=user&w='.SUser::id().'&v='.urlencode(SUser::name()).'" onclick="addHrefArg(this,[`s`,`st`])">'.qtSVG('user', 'class=btn-prefix').''.L('All_my_items').'</a>';
echo '</div>';